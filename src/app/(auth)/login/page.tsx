...
export default function LoginPage() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>("headquarters");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");

  // URLの ?error= パラメーターを読んでエラーを表示（Auth.jsリダイレクトエラー対応）
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const urlError = params.get("error");
    if (urlError === "Configuration") {
      setError(
        "ログイン設定にエラーがあります。管理者にお問い合わせください。"
      );
    } else if (urlError) {
      setError(`認証エラーが発生しました（${urlError}）`);
    }
  }, []);

  // 便利屋・エンドユーザー：LINEログイン
  // redirect: false を使い、LINE未設定の場合もlocalhost リダイレクトを防ぐ
  const handleLineLogin = async () => {
    ...
    if (result?.error) {
      setError(
        "LINEログインが現在利用できません。しばらくしてからお試しください。"
      );
      setIsLoading(false);
    } else if (result?.url) {
      // LINE認証ページへブラウザリダイレクト
  ...
  return (
    <div className="w-full max-w-md">
      ...
      {/* URLエラー / フォームエラー共通表示 */}
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4 text-sm text-red-600 text-center">
          {error}
        </div>
      )}
